...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    **Scratch evidence:** [RUN ID, `plan-review/before` OR CONCRETE REVISION DIFF]
    ...
    | Review+Fix | Confirms exactly one final review+fix agent owns all final edits after the quick-verified implementation checkpoint and before final verification. It uses effective `final_review_model`, directly receives in-scope fix authority, and does not create a concurrent reviewer or extra checkpoint. |
    ...
    Reject plans that omit the one final review+fix agent for the whole
    implementation, route it anywhere other than effective
    ...
    omit the six-base-key plus optional `review_model2` and
    `final_review_model` schema contract, define `SIMPLEPOWER_REVIEW_MODEL2` or
    `SIMPLEPOWER_FINAL_REVIEW_MODEL`, treat an absent `final_review_model` as
