...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    **Scratch evidence:** [RUN ID, `plan-review/before` OR CONCRETE REVISION DIFF]
    ...
    | Model Allocation | Confirms the active mandatory model tiers are exactly FAST/NORMAL/BEST/REVIEW and independent of `use_subagent`; every implementation task has FAST, NORMAL, or BEST; FAST defaults to `gpt-5.3-codex-spark-xhigh`, NORMAL defaults to `gpt-5.6-luna-max`, and BEST and REVIEW default to `gpt-5.6-sol-high`; FAST is limited to obvious repetitive/mechanical/static text/fixture or assertion churn and quick verification, NORMAL covers routine low-risk localized implementation work, BEST covers broad, ambiguous, behavior-shaping, high-risk, or hard-to-test implementation work, the primary plan reviewer is REVIEW-tier, the one primary final review+fix agent is REVIEW-tier, and the quick verifier uses FAST by default. Confirms an optional distinct `review_model2` is only a read-only secondary route, never a fifth mandatory tier or a second fixer. |
    | Model Resolution Precedence | Confirms the plan follows the six base keys plus optional `review_model2` in `skills/using-simplepower/references/simplepower-config.md`: built-in defaults, per-key overlays from `/home/gary/.codex/simplepower.toml`, repository `<git-root>/simplepower.toml`, the four non-empty `SIMPLEPOWER_REVIEW_MODEL`, `SIMPLEPOWER_BEST_MODEL`, `SIMPLEPOWER_NORMAL_MODEL`, and `SIMPLEPOWER_FAST_MODEL` process environment values, and explicit current-session instructions last. Confirms the six base TOML keys are `use_subagent`, `subagent_model`, `review_model`, `best_model`, `normal_model`, and `fast_model`; `review_model2` is optional, has no built-in default and no `SIMPLEPOWER_REVIEW_MODEL2`, resolves after the primary, and enables a secondary only when its fully resolved string is distinct rather than an exact match. Missing higher-layer keys inherit; every present TOML file is validated in full and is fatal if invalid even when a higher layer overrides its values; model assignments are not read from `AGENTS.md`; and each final dash suffix is one of `low`, `medium`, `high`, `xhigh`, `max`, or `ultra`. |
    ...
    | Review+Fix | Confirms exactly one primary REVIEW-tier review+fix agent owns all final edits after the quick-verified implementation checkpoint and before final verification. With a distinct secondary, the primary and secondary initial final reviews run concurrently against one snapshot with no edits, the coordinator consumes and synthesizes both reports, closes the secondary, and sends only the retained primary a self-contained in-scope fix authorization. Confirms the secondary never edits, creates files, commits, manages refs, reroutes, recurses, or becomes a concurrent writer. |
    | Reviewer Non-Recursion | Confirms every Simple Power dispatch uses `fork_turns="none"`, and the primary/secondary plan-review and final-review instructions require direct review in the current worker and forbid running Codex CLI, spawning subagents, invoking Simple Power skills, restarting execution, and rerouting the workflow. |
    ...
    Reject plans that omit the one primary REVIEW-tier review+fix agent for the
    whole implementation, permit a secondary to edit, or introduce a concurrent
    writer or extra checkpoint. Reject plans that omit combined approval, put the
    ...
    execution route. Reject plans that route the primary plan reviewer or
    primary final review+fix agent to BEST instead of REVIEW. Reject plans that
    do not state
    ...
    omit the six-base-key plus optional `review_model2` schema contract, define
    `SIMPLEPOWER_REVIEW_MODEL2`, or treat an absent or exact-match secondary as
    enabled. Reject
    ...
    plans whose plan reviewer, secondary reviewer, or final review+fix
    instructions allow running Codex CLI, spawning subagents, invoking Simple
    Power skills, restarting execution, rerouting the workflow, editing during
    a dual initial review, or delegating the assigned review instead of
