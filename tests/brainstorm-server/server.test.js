...
async function runTests() {
  ...
  try {
    ...
    await test('frame template has required structure', () => {
      ...
      assert(template.includes('Simple Power Brainstorming'), 'Should show Simple Power branding');
    ...
    await test('start-server.sh uses the Simple Power brainstorm session path', () => {
      const startServerScript = fs.readFileSync(
        path.join(__dirname, '../../skills/brainstorming/scripts/start-server.sh'), 'utf-8'
      );
      assert(startServerScript.includes('.simplepower/brainstorm'), 'Should persist sessions under .simplepower/brainstorm');
      return Promise.resolve();
    });
