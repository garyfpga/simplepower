...
require_contains 'skills/writing-plans/plan-document-reviewer-prompt.md' 'TOML keys are `review_model`, `best_model`, `normal_model`, and `fast_model`;' "plan reviewer checks per-key TOML overlay"
...
require_contains "skills/using-simplepower/references/codex-tools.md" 'layer replaces only the tier values it supplies. Do not read model assignments' "Codex tool mapping documents AGENTS retirement"
