...
#
# Sync this simplepower checkout → garyfpga/codex-plugins.
# Clones the marketplace repo fresh into a temp dir, rsyncs tracked upstream
# plugin content (including committed Codex files under .codex-plugin/ and
# assets/), maintains the marketplace index, commits, pushes a sync branch, and
# opens a PR.
...
#
# Bootstrap mode: skips the "plugin must exist on base" requirement, creates
# plugins/simplepower/ when absent, then copies the tracked plugin files from
# upstream and ensures the marketplace index includes Simple Power.
#
...
# =============================================================================
# Marketplace metadata helpers
# =============================================================================

ensure_marketplace_index() {
  local repo="$1"
  local index="$repo/$MARKETPLACE_REL"
...
simplepower_entry = {
    "name": "simplepower",
    "source": {"source": "local", "path": "./plugins/simplepower"},
...
if os.path.exists(index_path):
    ...
    data = {
        ...
        "interface": {"displayName": "Simple Power Codex Plugins"},
        "plugins": [],
    }
...
for i, plugin in enumerate(plugins):
    if isinstance(plugin, dict) and plugin.get("name") == "simplepower":
        plugins[i] = simplepower_entry
        break
else:
    plugins.append(simplepower_entry)
...
# =============================================================================
# Prepare destination (clone marketplace repo fresh, or use --local)
# =============================================================================
...
if [[ -n "$LOCAL_CHECKOUT" ]]; then
  ...
  echo "Cloning marketplace repo $FORK..."
...
copy_local_destination_overlay() {
  overlay_destination_paths "$DEST_REPO" < <(
    git -C "$DEST_REPO" diff --name-only -z -- "$DEST_REL" "$MARKETPLACE_REL"
  )
  overlay_destination_paths "$DEST_REPO" < <(
    git -C "$DEST_REPO" diff --cached --name-only -z -- "$DEST_REL" "$MARKETPLACE_REL"
  )
  overlay_destination_paths "$DEST_REPO" < <(
    git -C "$DEST_REPO" ls-files --others --exclude-standard -z -- "$DEST_REL" "$MARKETPLACE_REL"
  )
  overlay_destination_paths "$DEST_REPO" < <(
    git -C "$DEST_REPO" ls-files --others --ignored --exclude-standard -z -- "$DEST_REL" "$MARKETPLACE_REL"
  )
}

local_checkout_has_uncommitted_destination_changes() {
  [[ -n "$(git -C "$DEST_REPO" status --porcelain=1 --untracked-files=all --ignored=matching -- "$DEST_REL" "$MARKETPLACE_REL")" ]]
}
...
apply_to_preview_checkout() {
  ...
  ensure_marketplace_index "$PREVIEW_REPO"
}

preview_checkout_has_changes() {
  [[ -n "$(git -C "$PREVIEW_REPO" status --porcelain "$DEST_REL" "$MARKETPLACE_REL")" ]]
}
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  SYNC_BRANCH="bootstrap/simplepower-${UPSTREAM_SHORT}-${TIMESTAMP}"
else
  SYNC_BRANCH="sync/simplepower-${UPSTREAM_SHORT}-${TIMESTAMP}"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  echo "Mode:     BOOTSTRAP (creating plugins/simplepower/ when absent and ensuring marketplace metadata)"
fi
echo ""
...
echo ""
apply_to_preview_checkout
echo "=== Preview status (plugin files and marketplace metadata) ==="
if preview_checkout_has_changes; then
  git -C "$PREVIEW_REPO" status --short -- "$DEST_REL" "$MARKETPLACE_REL"
  echo ""
  echo "Preview diff summary:"
  git -C "$PREVIEW_REPO" add --intent-to-add "$DEST_REL" "$MARKETPLACE_REL" >/dev/null 2>&1 || true
  git -C "$PREVIEW_REPO" diff --stat -- "$DEST_REL" "$MARKETPLACE_REL" || true
else
  echo "No plugin file or marketplace metadata changes detected."
fi
echo "=== End preview status ==="
echo ""
...
if [[ -n "$LOCAL_CHECKOUT" ]]; then
  if local_checkout_has_uncommitted_destination_changes; then
    die "local checkout has uncommitted changes under '$DEST_REL' or '$MARKETPLACE_REL' — commit, stash, or discard them before syncing"
  fi

  apply_to_preview_checkout
  if ! preview_checkout_has_changes; then
    echo "No changes — plugin files and marketplace metadata were already in sync with garyfpga/simplepower $UPSTREAM_SHORT (v$UPSTREAM_VERSION)."
    exit 0
  fi
fi
...
ensure_marketplace_index "$DEST_REPO"

# Bail early if nothing actually changed
cd "$DEST_REPO"
if [[ -z "$(git status --porcelain "$DEST_REL" "$MARKETPLACE_REL")" ]]; then
  echo "No changes — plugin files and marketplace metadata were already in sync with garyfpga/simplepower $UPSTREAM_SHORT (v$UPSTREAM_VERSION)."
  exit 0
fi
...
git add "$DEST_REL" "$MARKETPLACE_REL"

if [[ $BOOTSTRAP -eq 1 ]]; then
  COMMIT_TITLE="bootstrap simplepower v$UPSTREAM_VERSION into garyfpga/codex-plugins @ $UPSTREAM_SHORT"
  PR_BODY="Initial bootstrap of the simplepower plugin into marketplace repo \`garyfpga/codex-plugins\` from upstream \`garyfpga/simplepower\` \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).

Creates \`plugins/simplepower/\` by copying the tracked plugin files from upstream, including \`.codex-plugin/plugin.json\` and \`assets/\`, and ensures \`.agents/plugins/marketplace.json\` includes the Simple Power entry.

Run via: \`scripts/sync-to-codex-plugin.sh --bootstrap\`
Marketplace repo: https://github.com/$FORK
Upstream commit: https://github.com/garyfpga/simplepower/commit/$UPSTREAM_SHA
...
This is a one-time bootstrap. Subsequent syncs will be normal (non-bootstrap) runs using the same tracked upstream plugin files."
  ...
  COMMIT_TITLE="sync simplepower v$UPSTREAM_VERSION into garyfpga/codex-plugins @ $UPSTREAM_SHORT"
  PR_BODY="Automated sync into marketplace repo \`garyfpga/codex-plugins\` from upstream \`garyfpga/simplepower\` \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).

Copies the tracked plugin files from upstream, including the committed Codex manifest and assets, and ensures \`.agents/plugins/marketplace.json\` includes the Simple Power entry.
...
Marketplace: https://github.com/$FORK
Upstream:    https://github.com/garyfpga/simplepower/commit/$UPSTREAM_SHA
Branch:      $SYNC_BRANCH"
